...
class ConfigFile:
    ...
    def relevant_config(self) -> Config:
        ...
        relevant_config: Config = cast(
            ...
            else self.toml_doc.unwrap().get("tool", {}).get("dragonfruit_tui", {}),
        )
    ...
    def update(self, config: Config) -> None:
        ...
        if self.is_pyproject:
            if "tool" not in self.toml_doc:
                self.toml_doc["tool"] = {"dragonfruit_tui": {}}
            elif "dragonfruit_tui" not in self.toml_doc["tool"]:  # type: ignore
                self.toml_doc["tool"]["dragonfruit_tui"] = {}  # type: ignore
            self.toml_doc["tool"]["dragonfruit_tui"].update(config)  # type: ignore
        else:
...
def _search_config() -> list[Path]:
    directory = user_config_path(appname="dragonfruit_tui", appauthor=False)
